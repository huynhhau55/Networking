...
public class Client1 {
	...
	static void sendFile() {
		...
		try {
			...
			Client1 = new Socket(InetAddress.getByName("172.16.1.2"),MasterServerPort);
			ListFile ds = new ListFile();
			ds.setId(0);
			ds.setPortNumber(Client1.getLocalPort());
			ds.setServerHost(Client1.getLocalAddress());
			ObjectOutputStream os = new ObjectOutputStream(Client1.getOutputStream());
			os.writeObject(ds);
						
			ObjectInputStream oi = new ObjectInputStream(Client1.getInputStream());
	...
	static void reciveFile(int portNumber,DatagramSocket socket,InetAddress add) throws ClassNotFoundException, IOException, NoSuchAlgorithmException {
		...
		try {
			...
			BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream("D:\\Client1\\" + fileInfo.getFilename()));
			...
			for (int i = 0; i < (fileInfo.getPiecesOfFile() - 1); i++) {
		     ...
		     String checkSumRecivedFile = GenerateCheckSumSHA.checkFile("D:\\Client1\\" + fileInfo.getFilename());
